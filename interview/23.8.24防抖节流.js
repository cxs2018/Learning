...
function debounce(fn, delay = 200, leading) {
  let timeout;
  return function () {
    timeout ? clearTimeout(timeout) : leading ? fn.call(this, ...arguments) : null
    timeout = setTimeout(fn.bind(this), delay, ...arguments)
  }
}

const handlerChange = debounce(function () { console.log('更新触发了') }, 1000, true)

// 绑定监听
document.querySelector("input").addEventListener('input', handlerChange);

function throttle(fn, threshhold = 200, leading) {
  let timeout;
  let start = new Date();
  return function () {
